...
# Mapa de Japeri situação de atendimento
ggplot(mapaMun_japeri) +
  geom_polygon(mapping=aes(x=long,y=lat,group=group), color="grey", fill="beige") +
  coord_map() +
  theme(plot.background=element_blank(), 
        ...
        axis.ticks=element_blank(), 
        axis.text=element_blank()) +
  geom_point(data = geocode_escolas_japeri, mapping = aes(x=lon, y = lat, color = str_to_title(rede), shape = str_to_sentence(Restrição.de.Atendimento))) +
  theme(legend.key=element_blank()) +
  scale_color_discrete(name="Dependência administrativa") +
  scale_shape_discrete(name = "Restrição de Atendimento") +
  ggtitle("Situação de atendimento das escolas em Japeri - RJ",
          subtitle="Fonte: INEP - Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira") 


escolas_publicas_japeri <- geocode_escolas_japeri %>% filter(rede == "estadual"|rede == "municipal")
escolas_exclusive_sem_matriculas <- geocode_escolas_japeri %>% filter(Porte.da.Escola != "")
...
ggplot(mapaMun_japeri) +
  ...
  theme(plot.background=element_blank(), 
        ...
        axis.ticks=element_blank(), 
        axis.text=element_blank()) +
  geom_point(data = escolas_exclusive_sem_matriculas, mapping = aes(x=lon, y = lat, color = Porte.da.Escola, shape = str_to_sentence(rede))) +
  theme(legend.key=element_blank()) +
  scale_color_discrete(name="Porte da escola", limits = c("Até 50 matrículas de escolarização", "Entre 51 e 200 matrículas de escolarização", "Entre 201 e 500 matrículas de escolarização",  "Entre 501 e 1000 matrículas de escolarização", "Mais de 1000 matrículas de escolarização")) +
  scale_shape_discrete(name = "Dependência administrativa") +
  ggtitle("Porte das escolas em Japeri - RJ",
          subtitle="Fonte: INEP - Instituto Nacional de Estudos e Pesquisas Educacionais Anísio Teixeira") 